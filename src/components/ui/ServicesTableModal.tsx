...
export function ServicesTableModal({ isOpen, onClose }: ServicesTableModalProps) {
  ...
  return (
    <AnimatePresence>
      <motion.div
        className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-2 xs:p-4"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        onClick={onClose}
      >
        <motion.div
          className="bg-white rounded-2xl shadow-2xl max-w-6xl w-full max-h-[90vh] overflow-auto relative"
        ...
        >
          ...
          <div className="sticky top-0 bg-white z-10 flex items-center justify-between p-4 xs:p-6 border-b border-beige-200">
            <h2 className="text-xl xs:text-2xl font-bold text-sage-900">
          ...
          <div className="p-4 xs:p-6 overflow-x-auto">
            <table className="w-full text-sm border-collapse min-w-[800px]">
              <thead>
                <tr>
                  <th className="text-left p-3 bg-sage-700 text-white font-semibold rounded-tl-lg sticky left-0 z-10 min-w-[180px]">
                    Что Вы получаете?
                  </th>
                  {serviceTableColumns.map((col, index) => (
                    <th
                      key={col.key}
                      className={`p-3 bg-sage-700 text-white font-semibold text-center min-w-[120px] ${
                        index === serviceTableColumns.length - 1 ? 'rounded-tr-lg' : ''
                      }`}
                    >
                      <div className="text-xs leading-tight">{col.label}</div>
                      <div className="text-sage-200 text-xs mt-1">{col.price}</div>
                    </th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {serviceTableRows.map((row, rowIndex) => (
                  <tr
                    key={rowIndex}
                    className={rowIndex % 2 === 0 ? 'bg-beige-50' : 'bg-white'}
                  >
