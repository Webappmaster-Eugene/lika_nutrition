...
test("verification covers the harness and the full quality gate of the single package", () => {
...
function nativeFixture() {
  return {
    configs: nativeExpected.cwds.map(cwd => ({ cwd, approval: "never", permissions: "lika", mcp: ["context7"] })),
...
test("standalone setup preserves memory and detects context, source and skill drift", () => {
  const sandbox = mkdtempSync(join(tmpdir(), "lika-harness-"));
  ...
  try {
    ...
    const retiredRoleSource = join(checkout, ".codex/context/lika/agents/retired-example.md");
    ...
    writeFileSync(join(checkout, ".codex/context/lika/conventions.md"), "corrupted\n");
    assert.ok(diagnose().some(message => message.includes("Generated file drift: .codex/context/lika/conventions.md")));
