...
export function handleEvent(event) {
  if (event.hook_event_name === "SessionStart") {
    ...
    return { hookSpecificOutput: { hookEventName: "SessionStart", additionalContext: `Lika Nutrition repository: ${repo}. Read AGENTS.md and .codex/context/CLAUDE.md before editing.\n\n${notes.join("\n\n")}` } };
  }
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  } catch { process.stdout.write(JSON.stringify({ systemMessage: "Project hook could not complete; run npm run codex:doctor manually." })); }
}
